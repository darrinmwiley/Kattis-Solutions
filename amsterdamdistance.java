...
public class amsterdamdistance {
	...
	public static void main(String[] args) throws Exception
	{
		...
		amsterdamdistance t = new amsterdamdistance();
	...
	public void run() throws Exception
	{
		...
		int x2 = file.nextInt();
		int y2 = file.nextInt();
		double vu = R/N;//vertical unit
		int xdiff = Math.abs(x1 - x2);//horizontal diff
		double theta = (180.0/M)*xdiff;
		double bestCandidate = Double.POSITIVE_INFINITY;
		for(int i = 0;i<=Math.max(y1,y2);i++)
		{
			double vd1 = Math.abs(i - y1)*vu;
			double hd = Math.PI*(i*vu*2)*theta/360;
			double vd2 = Math.abs(i - y2)*vu;
			bestCandidate = Math.min(bestCandidate, vd1+hd+vd2);
		}
