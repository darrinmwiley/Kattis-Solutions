		...
		for(int z = 0;z<zz;z++)
		{
			...
			for(int i = 3;i<dp.length;i+=2)
			{
				for(int j = 0;j<B+1;j++)
				{
					for(int k = 0;k<B+1;k++)
					{
						if((i & 1<<k) != 0 && j != k)//k'th location is visited
						{
			...
			for(int i = 1;i<B+1;i++)
			{
				min = Math.min(dp[dp.length - 1][i] + dist[i][0],min);
			}
