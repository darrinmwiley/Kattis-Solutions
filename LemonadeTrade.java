...
public class LemonadeTrade {
	...
	public static void main(String[] args) throws Exception
	{
		...
		LemonadeTrade t = new LemonadeTrade();
	...
	public void run() throws Exception
	{
		...
		for(int i = 0;i<N;i++)
		{
			String b = file.next();
			String a = file.next();
			double ratio = file.nextDouble();
			if(amount.containsKey(a))
			{
				if(amount.containsKey(b))
				{
					double max = Math.max(amount.get(b), amount.get(a)+Math.log(ratio));
					amount.put(b,max);
				}else {
					amount.put(b, amount.get(a)+Math.log(ratio));
				}
			}
		}
		if(amount.containsKey("blue")) {
			if(amount.get("blue") > Math.log(10))
				System.out.println(10);
			else
				System.out.println(Math.min(10, Math.exp(amount.get("blue"))));
		}
		else
